...
app.get('/averagesData', function (req, res) {
    ...
    connection.query("SELECT * FROM TTdb.user ORDER BY percentage DESC",
        function (err, rows, fields) {
            if (!err) {
                if (rows != '') {
                    ...
                    res.write('<tr><th class="w3-center w3-hide-small" colspan="6" style="width:100%;background-color:#f1f1f1;border-bottom:1px solid #f1f1f1;padding-bottom:10px">Division 1</th><th class="w3-hide-medium w3-hide-large w3-center" colspan="5" style="width:100%;background-color:#f1f1f1;border-bottom:1px solid #f1f1f1;padding-bottom:10px">Division 1</th></tr>');
                    ...
                    for (i = 0; i < rows.length; i++) {
                        ...
                        var pos = i + 1;
                        res.write('<tr><td class="w3-center">' + pos + '</td><td class="w3-center">' + rows[i].firstName + ' ' + rows[i].lastName + '</td><td class="w3-center w3-hide-small">' + rows[i].club + ' ' + rows[i].team + '</td><td class="w3-center">' + rows[i].gamesPlayed + '</td><td class="w3-center">' + rows[i].wins + '</td><td class="w3-center">' + percentConv + '%</td></tr>');
...
app.get('/profileData', function (req, res) {
    ...
    connection.connect();
    var username = req.query.u;
    connection.query("SELECT * FROM TTdb.user WHERE username = " + mysql.escape(req.query.u),
        function (err, rows, fields) {
            if (!err) {
                if (rows != '') {
                    res.writeHead(200, {
                    ...
                    for (i = 0; i < rows.length; i++) {
                        ...
                        var percentConv = percent.toFixed(2);
                        res.write('<table style="width: 80%;margin: 0 auto;" class="w3-table-all w3-card-4 w3-center">');
                        res.write('<tr><th class="w3-center" colspan="4" style="width:100%;background-color:#f1f1f1;border-bottom:1px solid #f1f1f1;padding-bottom:10px">' + rows[i].username + '</th></tr>');
                        res.write('<tr><th class="w3-center" style="width:25%">Played</th><th class="w3-center" style="width:25%">Wins</th><th class="w3-center" style="width:25%">%</th><th class="w3-center" style="width:25%">Position</th></tr>');
                        res.write('<tr><td class="w3-center">' + rows[i].gamesPlayed + '</td><td class="w3-center">' + rows[i].wins + '</td><td class="w3-center">' + percentConv + '%</td><td class="w3-center">23rd</td></tr>');
                        res.write('</table>');
                        res.write('<br>');
                        res.write('<table style="width: 80%;margin: 0 auto;" class="w3-table-all w3-card-4 w3-center">');
                        res.write('<tr><th class="w3-center" colspan="6" style="width:100%;background-color:#f1f1f1;border-bottom:1px solid #f1f1f1;padding-bottom:10px">' + rows[i].club + ' ' + rows[i].team + '</th></tr>');
                        res.write('<tr class="w3-hide-small"><th class="w3-center" style="width: calc(100% / 6);">Played</th><th class="w3-center" style="width: calc(100% / 6);">Wins</th><th class="w3-center" style="width: calc(100% / 6);">Draws</th><th class="w3-center" style="width: calc(100% / 6);">Losses</th><th class="w3-center" style="width: calc(100% / 6);">Points</th><th class="w3-center" style="width: calc(100% / 6);">Position</th></tr>');
                        res.write('<tr style="background-color: #f1f1f1;" class="w3-hide-large w3-hide-medium"><th class="w3-center" style="width: calc(100% / 6);">P</th><th class="w3-center" style="width: calc(100% / 6);">W</th><th class="w3-center" style="width: calc(100% / 6);">D</th><th class="w3-center" style="width: calc(100% / 6);">L</th><th class="w3-center" style="width: calc(100% / 6);">Pts</th><th class="w3-center" style="width: calc(100% / 6);">Pos</th></tr>');
                        res.write('<tr style="background-color: #fff;"><td class="w3-center">19</td><td class="w3-center">4</td><td class="w3-center">3</td><td class="w3-center">12</td><td class="w3-center">61</td><td class="w3-center">12th</td></tr>');
                        res.write('</table>');
                    }
                    res.end();
                    connection.end();
...
app.post('/editProfile', function (req, res) {
    if (req.method == 'POST') {
        ...
        req.on('end', function () {
            ...
            connection.connect();
            connection.query("UPDATE TTdb.user SET email = " + mysql.escape(post.email) + " WHERE username = " + mysql.escape(post.username),
                function (err, rows, fields) {
                    if (!err) {
                        connection.end();
...
app.get('/venueData', function (req, res) {
    ...
    connection.connect();
    connection.query("SELECT * FROM TTdb.clubs ORDER BY clubName;",
        function (err, rows, fields) {
            if (!err) {
                if (rows != '') {
                    res.writeHead(200, {
                    ...
                    res.write('<link rel="stylesheet" href="https://fonts.googleapis.com/css?family=Raleway">');
                    res.write('<link rel="stylesheet" href="w3.css">');
                    res.write('<table style="width: 95%;margin: 0 auto;" class="w3-table-all w3-card-4"><tr><th style="width:50%">Club</th><th style="width:50%">Address</th></tr>');
                    for (i = 0; i < rows.length; i++) {
                        res.write('<tr><td>' + rows[i].clubName + '</td><td>' + rows[i].clubAddress + '</td></tr>');
                    }
                    res.write('</table>');
                    res.write('<br>')
                    ...
                    for (; rows[j];) {
                        ...
                        if (rows[j].clubName == 'Saltash BEEC') {
                            rows[j].clubName = 'Headquarters';
                        }
                        res.write('markers[' + j + '].index = ' + j + ';contents[' + j + '] = "' + rows[j].clubName + '";');
...
app.post('/login', function (req, res) {
    if (req.method == 'POST') {
        ...
        req.on('end', function () {
            ...
            connection.connect();
            connection.query("SELECT * FROM TTdb.user WHERE username = " + mysql.escape(post.username) + "AND password = " + mysql.escape(post.password),
                function (err, rows, fields) {
                    if (!err) {
                        if (rows != '') {
